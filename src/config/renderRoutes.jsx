...
function withAuthCommonRoute(route) {
    ...
    const ret = args => {
        ...
        return <CommonRoute {...rest} render={props => <Component {...props} route={route} render={render} />} />;
    };
...
function wrapWithInitialProps(WrappedComponent, initialProps, extraProps = {}) {
    class WrappedInitialPropsComponent extends React.Component {
        ...
        render() {
            return <WrappedComponent {...{ ...this.props, ...this.state.extraProps }} />;
        }
    }
    ...
    return Array.isArray(routes) ? (
        <Switch {...switchProps}>
            {routes.map((route, i) => {
                ...
                const RoutesRoute = route.Routes ? withAuthCommonRoute(route) : CommonRoute;
                return (
                    <RoutesRoute
                        key={route.key || i}
                        path={route.path}
